...
- Claude usage supports the Claude Code OAuth usage API. Tokens are read from env, `~/.claude/.credentials.json`, or macOS Keychain, with `claude /usage` as a fallback.
...
- Claude 用量支持 Claude Code OAuth usage API；token 会从环境变量、`~/.claude/.credentials.json` 或 macOS Keychain 读取，最后 fallback 到 `claude /usage`。
...
- A logged-in `claude` CLI session, or `CLAUDE_ACCESS_TOKEN`; macOS Keychain credentials are supported automatically
